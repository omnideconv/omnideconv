...
DEAnalysisMAST<-function(scdata,id,path){
  ...
  colnames(data.used.log2)<-base::make.unique(colnames(data.used.log2))
  diff.cutoff=0.5
  for (i in unique(id)){
    ...
    bigtable <- data.frame(base::cbind(log2.stat.result, Auc))
    ...
    if(dim(DE)[1]>1){
      ...
      data_for_MIST <- as.data.frame(base::cbind(rep(rownames(counts), dim(counts)[2]), reshape::melt(counts),rep(groups, each = dim(counts)[1]), rep(1, dim(counts)[1] * dim(counts)[2]) ))
...
buildSignatureMatrixMAST<-function(scdata,id,path,diff.cutoff=0.5,pval.cutoff=0.01){
  ...
  for (i in unique(id)){
    if(file.exists(paste(path,"/",i,"_MIST.RData", sep=""))){
      ...
      pvalue_adjusted<-stats::p.adjust(cluster_lrTest.table[,3], method = "fdr", n = length(cluster_lrTest.table[,3]))
      cluster_lrTest.table<-base::cbind(cluster_lrTest.table,pvalue_adjusted)
