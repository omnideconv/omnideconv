...
#'
#' @param celltype_labels Vector of celltype labels. Order corresponds to rows in
...
build_model_scaden <- function(single_cell_object, celltype_labels, bulk_gene_expression = NULL,
  ...
  training_h5ad <- scaden_simulate(
    celltype_labels = celltype_labels, gene_labels = gene_labels,
  ...
  out <- tryCatch(
    {
      ...
      utils::write.table(bulk_data,
        file = bulk_data_tmp, sep = "\t", row.names = TRUE,
