---
title: "Immunedeconv2 Final Report"
...
\tiny
```{r showSigTable, echo=FALSE}
  knitr::kable(signatureMatrix[1:5,], digits=4)
```
\normalsize

We can use this signature matrix to run the deconvolution with Bisque. 

```{r deconvExample, eval=FALSE}
immunedeconv2::deconvolute(bulk, signatureMatrix, "bisque", single_cell_data, cell_type_annotations) 
```
\tiny
```{r showTable, echo=FALSE}
res <- immunedeconv2::deconvolute(bulk, signatureMatrix, "bisque", single_cell_data, cell_type_annotations) 
res <- data.frame(sample=c(rep("", nrow(res))), res)
...
```
```{r plots, echo=FALSE, out.width="50%", fig.ncol= 2}
...
```{r benchmarking, fig.width=6, fig.height=7, fig.align = 'center', eval=FALSE}
...
```
```{r benchmarkingFigure, echo=FALSE, out.width="50%"}
...
```{r benchmarkingFigureNonNormalized, echo=FALSE, out.width="50%"}
...
```{r benchmarkingInterchangeMatrix, echo=FALSE, out.width="100%"}
knitr::kable(data.frame(Deconv_method=c("Bisque", "CibersortX", "DWLS", "MOMF", "Scaden"), Bisque=c("yes", "(yes)", "(yes)", "yes", "-"), CibersortX=c("yes", "yes", "yes", "yes", "-"), DWLS=c("yes", "yes", "yes", "yes", "-"), MOMF=c("yes", "(yes)", "(yes)", "yes", "-"), Scaden=c("-", "-", "-", "-", "yes")), caption = "Interchangeability table: Row names determine the deconvolution method, column names determine which tool is used to build the signature matrix. '(yes)' indicates that intersecting the genes of bulk data, single cell data and signature matrix is necessary.")
```

```{r benchmarkingInterchangePlots, echo=FALSE, out.width="45%"}
knitr::include_graphics(c("images/ciber_matrices.png","images/dwls_matrices.png"))
```
```{r benchmarkingInterchangePlots2, echo=FALSE, out.width="45%"}
knitr::include_graphics(c("images/bisque_matrices.png","images/momf_matrices.png"))
```
