...
res_momf_melt <- melt(res_momf_2, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")
...
res_scaden_melt <- melt(res_scaden_2, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")
...
res_dwls_melt <- melt(res_dwls_2, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")
...
res_ciber_melt <- melt(res_ciber_2, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")

df <- dplyr::bind_rows(list(res_bisque_melt, res_momf_melt, res_dwls_melt, res_scaden_melt, res_ciber_melt))
df$cell_type <- gsub(" ", "_", df$cell_type)
names(RefData) <- c("T_cell", "Monocyte", "B_cell", "DC", "NK_cell")
RefData$sample <- rownames(RefData)
pivot_longer(RefData, !sample, names_to="cell_type", values_to="true_fraction") %>% merge(df, by=c("sample", "cell_type")) %>%
