...
load("../data/cell_type_annotations_small.RData")
load("../data/bulk_small.RData")
...
```{r showSc}
knitr::kable(single_cell_data_small[1:5, 1:4])
```

Now we build the signature matrix with Bisque and look at the values for the first few genes. 
```{r signatureExample}
signatureMatrix <- immunedeconv2::build_model(single_cell_data_small, cell_type_annotations_small, "bisque")
knitr::kable(signatureMatrix[1:5,])
```

We can use this signature matrix to run the deconvolution with Bisque. 
```{r deconvExample}
immunedeconv2::deconvolute(bulk_small, signatureMatrix, "bisque", single_cell_data_small, cell_type_annotations_small) %>%
knitr::kable()
```
...
df <- immunedeconv2::deconvolute(bulk_small, signatureMatrix, "bisque", single_cell_data_small, cell_type_annotations_small) %>%
reshape2::melt()
ggplot(df, aes(y=Var2, x=value, fill=Var1))+geom_bar(stat="identity", position = "stack")+labs(title="Bisque", y="sample", x="proportion", fill="cell type")+scale_fill_brewer(palette="Accent", breaks=c("B cell","Monocyte conventional","NK cell","T cell CD4","T cell CD8"))

ggplot(df, aes(y=Var2, x=value, color=Var1))+geom_point(size=3)+facet_wrap(~Var1)+labs(title="Bisque", y="sample", x="proportion", color="cell type")+scale_color_brewer(palette="Accent", breaks=c("B cell","Monocyte conventional","NK cell","T cell CD4","T cell CD8"))
```
...
immunedeconv2::build_model(single_cell_data_small, cell_type_annotations_small, "momf", bulk_small) %>%
immunedeconv2::deconvolute(bulk_gene_expression = bulk_small, method = "momf", single_cell_object = single_cell_data_small) %>%
reshape2::melt() %>%
ggplot(aes(y=Var1, x=value, fill=Var2))+geom_bar(stat="identity", position = "stack")+labs(title="MOMF", y="sample", x="proportion", fill="cell type")+scale_fill_brewer(palette="Accent", breaks=c("B cell","Monocyte conventional","NK cell","T cell CD4","T cell CD8"))

# immunedeconv2::build_model(single_cell_object = single_cell_data_small, cell_type_annotations = cell_type_annotations_small, method = "scaden", bulk_gene_expression = bulk_small) %>%
# immunedeconv2::deconvolute(bulk_gene_expression = bulk_small, method = "scaden", single_cell_object = single_cell_data_small) %>%
# reshape2::melt() %>%
# ggplot(aes(y=Var1, x=value, fill=Var2))+geom_bar(stat="identity", position = "stack")+labs(title="Scaden", y="sample", x="proportion", fill="cell type")+scale_fill_brewer(palette="Accent", breaks=c("B cell","Monocyte conventional","NK cell","T cell CD4","T cell CD8"))
