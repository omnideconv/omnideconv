...
## Bisque
Bisque takes advantage of matched bulk and scRNA-seq samples to improve accuracy of cell-type estimates. Using linear regression, it corrects for platform-specific biases between pseudo-bulk reference profiles derived from scRNA-seq and bulk RNA-seq samples. It then applies NNLS-regression to deconvolute other bulk RNA-seq samples.
...
## MOMF
With MOMF, Multi-Omics Matrix Factorization, models the cell type specific mean in sc RNA-seq data by also accounting for the uncertainty of these cell type specific mean gene expression levels. Alternating Direction Method of Multipliers algorithm is then used to estimate the parameters in bulk RNA-seq downstream analyses. 
...
# 4. Benchmarking
To evaluate the performance of the methods used, we compared the whole data set with 17k cells from Hoek et al. [@Hoek2015] to its ground truth that was measured with FACS. This testset is also included in a benchmarking method, which takes the desired filename and -type as a parameter.
...
```
```{r benchmarkingFigure, echo=FALSE, out.width="60%"}
...
```{r benchmarkingFigureNonNormalized, echo=FALSE, out.width="60%"}
...
The table below gives an overview of signature matrices built with which of the methods (from the column names) can be used for deconvolution with which other method (from the rownames). For instance, a signature matrix built with Bisque can be used for almost all deconvolution methods (look in the column named Bisque), whereas deconvolution with Bisque only works with signature matrices from Bisque or MOMF (look in the row named Bisque).

Unfortunately, not all deconvolution methods can use all signature matrices, which sets a limit to the possible improvements by interchanging signature matrices between methods. The problem is that if the matrices are changed to acommodate all methods, information is lost as they need to be brought in another format.
...
As can be concluded from this figure, interchanging matrices between the methods does not considerably improve the predictions. One might derive small improvements especially when building the signature matrix with Bisque, but the reason behind this needs further examination in the future.
