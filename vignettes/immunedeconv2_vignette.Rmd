...
# 1. Background
In the first section, we will give some background information about the problem, why cell-type deconvolution is necessary and how it works.
...
In this section, we want to present our package immunedeconv2 by giving an example workflow. 
...
Note: Bisque and MOMF additionally need the single cell RNA-seq matrix you used to build the signature matrix for their deconvolution. As a fifth parameter, Bisque also needs cell type annotations, which were also already needed for building the model. 
...
To give you a feeling for the data, this is what a few rows and columns of our small single cell RNA-seq dataset look like. This specific selection of rows and columns was performed as the first rows and columns just show zeros. Do not be alarmed by a great number of zeros in your single cell data, this effect is normal:
...
Additionally, we provid some methods for visualisation. In this and the benchmarking section you can find examples of how to use them. For the plotDeconvResult method, you can additionally provide a method and file name to add a title to your plot and save it.
...
knitr::include_graphics("/home/nicolas/Uni/WS2021/SysMed/project/immunedeconv2/vignettes/images/methods_table.png")
```
## Bisque
...
# 4. Benchmarking
The deconvolution results displayed in section 2 are analysed in more detail in this section. 
As our test data set only includes a limited number of cells, the figures shown in this section are based on the whole data set with 17k cells from Hoek et al.

We also provide a method to compare the deconvolution results of all tools with a ground truth, that was measured with FACS by Hoek et al.
...
As already addressed in section 1, first-generation deconvolution tools use fixed, internal signature matrices containing only a limited set of human cells. The second-generation tools should bring more flexibility into this as they allow deriving cell-type specific signature matrices. The advantage of fixed signature matrices is, that they can be modulated to fit to the bulk data very well. This is, why quanTIseq and EPIC produce very good predicitions in comparison to Scaden. However, other datasets might not be suitable for these two methods. Furthermore, Scaden does not fall off behind regarding its predictions, especially when compared to EPIC. Even though its predictions are not perfect, the future of second-generation and therefore deconvolution of many different cell types seems very promising.
...
The table below gives an overview of signature matrices built with which of the methods (from the column names) can be used for deconvolution with which other method (from the rownames). For instance, a signature matrix built with Bisque can be used for almost all deconvolution methods (look in the column named Bisque), whereas deconvolution with Bisque only works with signature matrices from Bisque or MOMF (look in the row named Bisque). Unfortunately, not all deconvolution methods can use all signature matrices, which sets a limit to the possible improvements by interchanging signature matrices between methods. 

```{r benchmarkingInterchangeMatrix, echo=FALSE, out.width="80%"}
knitr::include_graphics("images/interchangeMatrix.png")
```
...
As can be concluded from this figure, interchanging matrices between the methods does not considerably improve the predictions, at least not for our dataset. 
