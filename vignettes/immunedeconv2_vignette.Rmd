...
ggplot(aes(y=samples, x=predicted_fraction, fill=cell_type))+geom_bar(stat="identity", position = "stack")+labs(title="Bisque", y="sample", x="predicted fraction", fill="cell type")+scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
ggsave("vignettes/images/bisque_prop.png")

reshape2::melt(res_bisque) %>% set_colnames(c("samples", "cell_type", "predicted_fraction")) %>%
ggplot(aes(y=samples, x=predicted_fraction, color=cell_type))+geom_point(size=3)+facet_wrap(~cell_type)+labs(title="Bisque", y="sample", x="predicted fraction", color="cell type")+scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
```
...
```{r otherMethods, fig.width=7, fig.height=4, results='hide'}
...
ggplot(aes(y=samples, x=predicted_fraction, fill=cell_type))+geom_bar(stat="identity", position = "stack")+labs(title="MOMF", y="sample", x="proportion", fill="cell type")+scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
ggsave("momf_prop.png")

res_scaden <- immunedeconv2::build_model(single_cell_object = single_cell_data, cell_type_annotations = cell_type_annotations, method = "scaden", bulk_gene_expression = bulk) %>%
...
ggplot(aes(y=samples, x=predicted_fraction, fill=cell_type))+geom_bar(stat="identity", position = "stack")+labs(title="Scaden", y="sample", x="proportion", fill="cell type")+scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
ggsave("scaden_prop.png")
...
ggplot(aes(y=samples, x=predicted_fraction, fill=cell_type))+geom_bar(stat="identity", position = "stack") + labs(title="DWLS", y="sample", x="proportion", fill="cell type") + scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
ggsave("vignettes/images/dwls_prop.png")
...
ggplot(aes(y=samples, x=predicted_fraction, fill=cell_type))+geom_bar(stat="identity", position = "stack") + labs(title="CibersortX", y="sample", x="proportion", fill="cell type") + scale_fill_brewer(palette="Paired", breaks=c("B cell","Monocyte conventional", "Monocyte non-conventional","NK cell","T cell CD4","T cell CD8", "T cell dividing", "T cell regulatory"))
ggsave("vignettes/images/cibersortx_prop.png")
...
## Benchmarking
In the following scetion, we compare the deconvolution results of all methods with the ground truth, that was measured with FACS by Hoek et al.
...
```{r benchmarking, echo=FALSE, fig.width=6, fig.height=7}
tcells <- c("T cell CD4", "T cell CD8", "T cell regulatory", "T cell dividing")
samples <- rownames(res_bisque)
res_bisque <- as.data.table(res_bisque)
res_bisque$T_cell <- rowSums(res_bisque[,tcells])
res_bisque$Monocyte <- rowSums(res_bisque[,c("Monocyte conventional", "Monocyte non-conventional")])
res_bisque$sample <- samples
res_bisque$method <- rep("Bisque", nrow(res_bisque))
res_bisque_melt <- melt(res_bisque, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")

res_momf <- as.data.table(res_momf)
res_momf$T_cell <- rowSums(res_momf[,tcells])
res_momf$Monocyte <- rowSums(res_momf[,c("Monocyte conventional", "Monocyte non-conventional")])
res_momf$sample <- samples
res_momf$method <- rep("MOMF", nrow(res_momf))
res_momf_melt <- reshape2::melt(res_momf, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")

res_scaden <- as.data.table(t(res_scaden))
res_scaden$T_cell <- rowSums(res_scaden[,tcells])
res_scaden$Monocyte <- rowSums(res_scaden[,c("Monocyte.conventional", "Monocyte.non.conventional")])
res_scaden$sample <- samples
res_scaden$method <- rep("Scaden", nrow(res_scaden))
res_scaden_melt <- reshape2::melt(res_scaden, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")

res_dwls <- as.data.table(res_dwls)
res_dwls$T_cell <- rowSums(res_dwls[,tcells])
res_dwls$Monocyte <- rowSums(res_dwls[,c("Monocyte_conventional", "Monocyte_non-conventional")])
res_dwls$sample <- samples
res_dwls$method <- rep("DWLS", nrow(res_dwls))
res_dwls_melt <- reshape2::melt(res_dwls, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")

res_ciber <- as.data.table(res_ciber)
res_ciber$T_cell <- rowSums(res_ciber[,tcells])
res_ciber$Monocyte <- rowSums(res_ciber[,c("Monocyte_conventional", "Monocyte_non-conventional")])
res_ciber$sample <- samples
res_ciber$method <- rep("CibersortV", nrow(res_ciber))
res_ciber_melt <- reshape2::melt(res_ciber, id.vars = c("sample", "method"), variable.name = "cell_type", value.name = "predicted_fraction")
