...
# Load packages needed for the visualisation section
library(dplyr)
library(tidyr)
library(ggplot2)
library(tibble)
...
# Load pre-computed deconvolution results stored in vignette_files/
vf <- if (file.exists("vignettes/vignette_files")) "vignettes/vignette_files" else "vignette_files"
deconvolution.results.dwls <- readRDS(file.path(vf, "deconvolution.results.dwls.rds"))
deconvolution.results.bayesprism <- readRDS(file.path(vf, "deconvolution.results.bayesprism.rds"))
deconvolution.results.dwls.minor <- readRDS(file.path(vf, "deconvolution.results.dwls.minor.rds"))

# Load patient metadata (Wu et al. 2021 Supplementary Table 1)
patient.metadata <- read_excel(file.path(vf, "41588_2021_911_MOESM4_ESM.xlsx"),
  sheet = 1, skip = 3
) %>%
...
patient.metadata <- patient.metadata %>%
  filter(Sample %in% rownames(deconvolution.results.dwls))

# Prepare long-format results for the major resolution
patients.results <- rownames_to_column(as.data.frame(deconvolution.results.dwls), "Sample") %>%
  ...
  left_join(patient.metadata, by = "Sample")

# Prepare long-format results for the minor resolution
...
We will need to download and unzip the datasets (`GSE176078_Wu_etal_2021_BRCA_scRNASeq.tar.gz`, `GSE176078_Wu_etal_2021_bulkRNAseq_raw_counts.txt.gz`), and store them in a data directory.
For this example analysis, we will also need the additional clinical information about the patients, available as [Supplementary Table 1](https://static-content.springer.com/esm/art%3A10.1038%2Fs41588-021-00911-1/MediaObjects/41588_2021_911_MOESM4_ESM.xlsx) from the paper.
...
- $MAD$ is the Median Absolute Deviation, computed as $MAD = median(|X_i - median(X)|)$
...
We can also visualise the results as a barplot through the built-in `plot_deconvolution` function:

```{r dwls-plot, eval=FALSE}
omnideconv::plot_deconvolution(list("dwls" = deconvolution.results.dwls), "bar", "method", "Spectral")
```

### 6.2 Deconvolution with BayesPrism

The third method we will use is [BayesPrism](https://github.com/Danko-Lab/BayesPrism) [@Chu2022]. This method is based on a Bayesian framework and models the transcriptomic expression observed in the scRNA-seq dataset. It then uses this information to dissect the bulk RNA-seq.
...
```{r minor-deconvolute, eval=FALSE}
counts.matrix <- as.matrix(single.cell.data.sampled@assays$RNA@counts)
...
```{r plot-treatment, fig.width=10, fig.height=6}
patients.results %>%
...
```{r plot-tcells, fig.width=8, fig.height=5}
...
```{r plot-cafs, fig.width=8, fig.height=5}
patients.results.minor %>%
...
```{r plot-cancer, fig.width=10, fig.height=5}
patients.results.minor %>%
