...
After building the specific signature matrix, you can calculate the cell properties in bulk RNA-seq data with the deconvolute function. For this, you need to provide the bulk data, signature matrix and you can, again, choose between the methods listed in the previous section. 

```{r deconv, eval=FALSE}
omnideconv::deconvolute(bulk, signature_matrix, method)
```

Note: Bisque and MOMF additionally need the single cell RNA-seq matrix you used to build the signature matrix for their deconvolution.
...
cell_type_annotations <- readRDS("../data/cell_type_annotations_for_vignette.rds")
single_cell_data <- readRDS("../data/single_cell_data_for_vignette.rds")
batch_ids <- readRDS("../data/batch_ids_for_vignette.rds")
...
```{r showSc}
knitr::kable(single_cell_data[101:105, 1:3], caption = "Table 1: Example of the raw single cell counts.")
```
...
result_list <- list(Bisque = res_bisque, CibersortX = res_ciber, DWLS = res_dwls, MOMF = res_momf, Scaden = res_scaden)
