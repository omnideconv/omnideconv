...
devtools::install_github("grst/omnideconv")
```
...
```{r signatureExample}
signatureMatrix <- omnideconv::build_model(single_cell_data, cell_type_annotations, "bisque")
...
```{r deconvExample}
omnideconv::deconvolute(bulk, signatureMatrix, "bisque", single_cell_data, cell_type_annotations) %>%
...
res_bisque <- omnideconv::deconvolute(bulk, signatureMatrix, "bisque", single_cell_data, cell_type_annotations)
...
```
```{r plots2, echo=FALSE, out.width="80%", fig.cap="Figure 4: Results of the example workflow in 2.6 but with other algorithms. The results differ from the bisque cell proportions and show more homogeneity. MOMF, DWLS and CibersortX predict mostly conventional monocytes, while Scaden shows a very mixed picture."}
